...
from models.Tradeoff.utils import create_users_adjacency_matrix, create_items_adjacency_matrix, create_dicts, \
    get_user_means, remove_user_means, add_user_means, get_graph_types, constraint_in_range


def open_graphs(data_dir, dataset, eval_ratio, test_ratio, uim, k, corr_mat, dict_to_use, method, mode):
    base_path = os.path.join(data_dir, dataset, 'tradeoff', 'matrices')
    if mode == 'similarity':
        if method == 'USERS':
            path = os.path.join(base_path, f'item_specific_graphs_k_{k}_{int((1-eval_ratio-test_ratio)*100)}')
        else:
            path = os.path.join(base_path, f'user_specific_graphs_k_{k}_{int((1-eval_ratio-test_ratio)*100)}')
    else:
        if method == 'USERS':
            path = os.path.join(base_path, f'DIS_item_specific_graphs_k_{k}_{int((1-eval_ratio-test_ratio)*100)}')
        else:
            path = os.path.join(base_path, f'DIS_user_specific_graphs_k_{k}_{int((1-eval_ratio-test_ratio)*100)}')
...
def adj_matrices(uim, corr_mat, dict, k, path, method, mode):
    ...
    for cnt, element in enumerate(tqdm(range(0, size))):
        # if cnt % 100 == 0:
...
#     print('-------- TRAINING --------')
#     for cnt_process, (user, item, rating) in enumerate(tqdm(train)):
...
#     return x_ui, x_uj
#
# def general_double(data_dir, ml_size, train_set,  k_most_similar, k_most_dissimilar, sim_ord, dis_ord, alpha, mu, method):
#     path = f'{data_dir}/{ml_size}/tradeoff/matrices/'
#
...
#     # user_id)
#     user_dict, item_dict = create_dicts(train_set)
#
...
#         method2 = 'USERS'
#
#         graphs1 = open_graphs(data_dir, ml_size, uim, k_most_similar, B, item_dict, method1, 'similarity')
#         graphs2 = open_graphs(data_dir, ml_size, uim, k_most_dissimilar, B, item_dict, method2, 'dissimilarity')
#     elif method == 'II':
#         method1 = 'ITEMS'
#         method2 = 'ITEMS'
#
#         graphs1 = open_graphs(data_dir, ml_size, uim, k_most_similar, C, user_dict, method1, 'similarity')
#         graphs2 = open_graphs(data_dir, ml_size, uim, k_most_dissimilar, C, user_dict, method2, 'dissimilarity')
#     elif method == 'UI':
#         method1 = 'USERS'
#         method2 = 'ITEMS'
#
...
#         A, y = training_matrices(alpha, uim, train_set, graphs1, graphs2, sim_ord, dis_ord, method1, method2,
#                                  means)
#
